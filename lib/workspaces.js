...
import { BRAND_NAME, BRAND_FULL, APP_URL } from './brand.js';
...
export async function cancelWorkspaceInvitation({ ctx, workspaceId, invitationId }) {
  const db = getDB();
  const mem = await membershipOf(db, workspaceId, ctx.userId);
  if (!mem || !MANAGE_ROLES.has(mem.role)) throw wsError('Only an owner or admin can cancel invitations.', 403);
  const { data, error } = await db.from('invitations')
    .delete().eq('id', invitationId).eq('workspace_id', workspaceId).is('accepted_at', null)
    .select('id').maybeSingle();
  if (error) throw wsError('Could not cancel the invitation.', 500);
  if (!data)  throw wsError('No matching pending invitation.', 404);
  return { cancelled: true, invitation_id: invitationId };
}

// Delete an entire org workspace (owner only). Drops the namespace and the
// workspace's tenant, which cascades the workspace, memberships, and its items.
export async function deleteWorkspace({ ctx, workspaceId }) {
  const db = getDB();
  const mem = await membershipOf(db, workspaceId, ctx.userId);
  if (!mem) throw wsError('Workspace not found.', 404);
  const { data: ws } = await db.from('workspaces').select('id, owner_id, tenant_id, type').eq('id', workspaceId).maybeSingle();
  if (!ws) throw wsError('Workspace not found.', 404);
  if (ws.type !== 'organisational') throw wsError('Only organisation workspaces can be deleted here.', 400);
  if (ws.owner_id !== ctx.userId) throw wsError('Only the owner can delete a workspace.', 403);
...
function buildWorkspaceEmail({ to, workspaceName, inviter, token }) {
  ...
  const heading = `${workspaceName} on ${BRAND_NAME}`;
  ...
  return {
    to,
    subject: token ? `Join ${workspaceName} on ${BRAND_NAME}` : `You were added to ${workspaceName}`,
...
function minimalBrandHtml({ heading, body, ctaUrl, ctaLabel }) {
  ...
  <div style="max-width:480px;margin:0 auto;padding:36px 28px;">
    <div style="font-size:13px;letter-spacing:.08em;text-transform:uppercase;color:#857C6E;margin-bottom:18px;">${BRAND_FULL}</div>
