...
export const compileConceptsJob = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const outcome = await step.run('mark-stale-or-bootstrap', async () => {
      ...
      if (!existing?.length) {
        // Bootstrap — first compile for this tenant runs the full pipeline.
        // compileConceptsForTenant writes its own background_log entry and
        // THROWS on fatal failure (Inngest retries), so no eternal "compiling".
      ...
      const { markStalePagesForItem, compileConceptsForTenant } = await import('./compile-concepts.js');
      try {
        ...
        // Before migration 024 the is_stale/stale_since columns don't exist.
        // Preserve the prior behaviour (full recompile) so nothing regresses in
        // the deploy → migrate window; the cheap path takes over once migrated.
        if (/is_stale|stale_since|column|schema cache/i.test(err?.message || '')) {
          await compileConceptsForTenant({ userId: user_id, tenantId: tenant_id });
          return { mode: 'fallback-full' };
        }
        throw err;
      }
    });

    // Schedule a debounced recompile only when something actually went stale.
    // The debounce collapses bursts (bulk contribute, document chunking) into a
    // single recompile run per tenant.
    if (outcome?.mode === 'mark-stale' && outcome.staleCount > 0) {
